...
Github에서 awesome css frameworks를 검색해보면, [{{ 'github-circle' | mdi | safe }}troxler/awesome-css-frameworks](https://github.com/troxler/awesome-css-frameworks/) 저장소가 제일 위에 나온다. 이 중에서 [Bootstrap][bootstrap], [Milligram][milligram], [Bulma][bulma] 같은 프레임워크들을 볼 수 있을 것이다.
...
한가지 주의할 것은, 앞에서 참조했던 [{{ 'github-circle' | mdi | safe }}11ty/eleventy-base-blog](https://github.com/11ty/eleventy-base-blog)의 기본 설정으론 html 파일은 [Nunjucks][nunjucks]를 쓰지만 md 파일은 [Liquid][liquid]로 지정되어 있다. 템플릿 엔진을 이것저것 쓰면 정신사나우니 이걸 [Nunjucks][nunjucks]로 바꿔서 쓰도록 하자. 바꾸고 싶을 때는 `.eleventy.js` 파일을 수정하면 된다.
...
제일 먼저 해야할 일은, 기존 css를 비활성화시키고 [Bulma][bulma]를 추가하는 일이다. 빨간 줄은 지우고 초록색 줄은 추가하면 된다.
...
이제 `toc` 필터를 이용할 수 있게 되었다. 다음 코드를 `_includes/layouts/post.njk`에 추가하도록 하자. `{% raw %}{{ content | safe }}{% endraw %}` 위에 넣어주어야 목차가 본문 위에 위치할 수 있으므로 참고하자.
